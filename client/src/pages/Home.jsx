  ...
  return (
    <div className="min-h-screen bg-white text-slate-800">
      ...
      <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/90 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2">
            <span className="font-bold text-indigo-600">SplitEase</span>
          </Link>
      ...
      <section className="max-w-6xl mx-auto px-6 pt-20 pb-16 text-center">
        ...
        <h1 className="text-4xl sm:text-5xl font-extrabold text-slate-900 tracking-tight leading-tight mb-5 max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-center gap-3">
          ...
          <a href="#demo" className="text-sm text-slate-600 hover:text-slate-900 px-6 py-3 rounded-xl border border-slate-200 hover:border-slate-300 transition">
            See how it works
          </a>
        </div>
      </section>

      <section id="demo" className="max-w-6xl mx-auto px-6 pb-24 scroll-mt-20">
            ...
            <div className="space-y-1">
              {Object.entries(demos).map(([key, d]) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg text-sm transition ${activeTab === key
                  ...
                  {demo.members.map((name, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-2.5">
                        <div className={`w-8 h-8 rounded-full ${colors[i % colors.length]} flex items-center justify-center text-white text-xs font-medium`}>
                          {name.slice(0, 2).toUpperCase()}
                        </div>
                        <div>
                          <p className="text-sm font-medium text-gray-900">{name}</p>
                          <p className="text-xs text-gray-400">{name.toLowerCase().replace(' ', '')}@example.com</p>
                        </div>
                      </div>
                    );
                  })}
                  ...
                  {demoTab === 'expenses' && (
                    <div className="space-y-3">
                      {demo.expenses.map((exp, i) => (
                        <div key={i} className="flex items-center justify-between p-3.5 bg-white rounded-xl border border-gray-100 hover:shadow-sm transition">
                          <div className="flex items-center gap-3">
                          ...
                          <div key={i} className="flex items-center justify-between p-3.5 bg-white rounded-xl border border-gray-100">
                            <div className="flex items-center gap-2.5">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-gray-400">{b.from.toLowerCase()}@example.com</p>
                              </div>
                            </div>
                            <div className="text-right">
                              {b.balance !== 0 && (
      ...
      <section className="py-20">
        <div className="max-w-2xl mx-auto px-6 text-center">
          ...
          <Link
            to="/register"
            className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-8 py-4 rounded-xl transition"
          >
      ...
      <footer className="border-t border-slate-200 py-8">
        <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-slate-400">
          ...
          <p>Built by <span className="text-slate-600 font-medium">Vidhi Patel</span> · MERN Stack Project</p>
